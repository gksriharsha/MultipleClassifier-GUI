...
def classify(paths):
    ...
    if(Encoder_settings != "Null"):
        if(Encoder_settings == "OHE"):
            ...
            raw_file1 = checks.text2numberOHE(
                paths[0], columns=list(settings["Encoder_OHE"]))
            raw_file = checks.text2number(
                data=raw_file1, columns=list(settings["Encoder_LE"]))
        else:
            pass
    ...
    json.dump(conf_matrix.tolist(), codecs.open('/conf.json', 'w',
...
def classify_KNN(classifier_settings):
    ...
    if(bool(classifier_settings)):
        clf_KNN = KNeighborsClassifier(
            n_neighbors=classifier_settings['K'], weights=classifier_settings['weights'])
    else:
...
def classify_SVM(classifier_settings):
    ...
    if(bool(classifier_settings)):
        clf_SVM = SVC(degree=int(classifier_settings["Degree"]), tol=float(
            classifier_settings["tol"]))
    else:
        clf_SVM = SVC()
