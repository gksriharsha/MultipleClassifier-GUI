...
def classifier_selection():
    ...
    for classifier, value in classifiers:
        ttk.Radiobutton(classifierframe, text=classifier, variable=selectedclassifier,
                        value=value, command=radio_settings).pack(pady=5, anchor=W)
...
def file_selection():
    ...
    data_path = ttk.Entry(fileframe, state='readonly',
                          width=80, textvariable=file_path_data)
    label_path = ttk.Entry(fileframe, state='readonly',
                           width=80, textvariable=file_path_labels)

    def data_openfile():
        global paths
        root.filename = filedialog.askopenfilename(
            initialdir="/", title="Select Data for Classification", filetypes=(("csv files", "*.csv"), ("all files", "*.*")))
    ...
    def file_check():
        ...
        if(not (paths[0] == '')):
            ...
            if((checks.contains_text_check(paths[0])) and (checks.getTextcols(paths[0]) != [])):
                ...
                LE_cols = []
                Colselect = Entry(
                    ErrorWindow, textvariable=columnVar, state='readonly')
                ...
                def encselect2(columns):
                    ...
                    columns = np.setdiff1d(
                        np.setdiff1d(columns, OHE_cols), LE_cols)
                    if(columns.size == 0):
                        columnVar.set(None)
                        ...
                        columnVar.set(columns[0])
                    Encoderoptn_select.set(None)
                B = ttk.Button(ErrorWindow, text="Submit",
                               command=lambda: encselect2(columns), state=DISABLED)
                R1 = ttk.Radiobutton(ErrorWindow, text="Text labels do not have a relationship",
                                     variable=Encoderoptn_select, value="OHE", state=DISABLED)
                R2 = ttk.Radiobutton(ErrorWindow, text="Text labels do have a relationship",
                                     variable=Encoderoptn_select, value="LE", state=DISABLED)
                Close = ttk.Button(
                    ErrorWindow, text="Close Window", command=ErrorWindow.destroy)
                ttk.Radiobutton(ErrorWindow, text="Text labels do not have a relationship",
                                variable=Encoderoptn, value="OHE", command=encselect).pack(anchor=W)
                ttk.Radiobutton(ErrorWindow, text="Text labels do have a relationship",
                                variable=Encoderoptn, value="LE", command=encselect).pack(anchor=W)
                if(len(columns) > 20):
                    ttk.Radiobutton(ErrorWindow, text="Choose for each column seperately",
                                    variable=Encoderoptn, value="OHE/LE", state=DISABLED).pack(anchor=W)
                else:
                    ttk.Radiobutton(ErrorWindow, text="Choose for each column seperately",
                                    variable=Encoderoptn, value="OHE/LE", state=NORMAL, command=encselect).pack(anchor=W)
    ...
    def label_openfile():
        global paths
        root.filename = filedialog.askopenfilename(
            initialdir="/", title="Select Labels for Classification", filetypes=(("csv files", "*.csv"), ("all files", "*.*")))
    ...
    ttk.Radiobutton(fileframe, text="Data and Labels in single file", variable=fileoptn,
                    value="One file", command=lambda: file_number_choose(fileoptn)).pack(anchor=W)
    ttk.Radiobutton(fileframe, text="Data and Labels as seperate files", variable=fileoptn,
                    value="Two files", command=lambda: file_number_choose(fileoptn)).pack(anchor=W)

    data_button = ttk.Button(
        fileframe, text="Open Data file", command=data_openfile)
    label_button = ttk.Button(
        fileframe, text="Open Labels file", command=label_openfile)
    Insights_button = ttk.Button(fileframe, text="Dataset Properties",
                                 command=lambda: insights.getInsights(paths[0]), state=DISABLED)
...
def mini_results():
    ...
    ttk.Label(miniresultsFrame, text="Classifier used:    ").grid(
        row=0, column=0, pady=10)
    Entry(miniresultsFrame, textvariable=classifier, state='readonly',
          width=10, relief=FLAT).grid(row=0, column=1, pady=10)
    Entry(miniresultsFrame, textvariable=classifier1, state='readonly',
          width=10, relief=FLAT).grid(row=0, column=2, pady=10)
    ttk.Label(miniresultsFrame, text="Accuracy:    ").grid(row=1, column=0)
    Entry(miniresultsFrame, textvariable=accuracy, relief=FLAT,
          state='readonly').grid(row=1, column=1, pady=10, columnspan=2)

    resultsbutton = ttk.Button(miniresultsFrame, text="Detailed Results",
                               command=lambda: results.showresults(), state=DISABLED)
...
def click_button():
    ...
    myButton = ttk.Button(root, text="Classify",
                          state=DISABLED, command=classify)
